...
| 9. Architecture Decisions (ADRs) | Two ADRs needed: (a) where ownership is enforced — data-access layer vs. per query, per NFR-002; (b) checked-in seed data vs. TMDB fetch at seed time, per §3.1. Both extend the store ADR already owed by 001-002. `docs/architecture/adr/` still does not exist |
...
| --- | ----------------------------------------------------------------------------------------------------------------------------------------- | ------------ | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | FR-015 measures "visibly different taste" via top-three recommendations, but story 013 supplies those. How is it verified before 013?     | tjanfei chao | Open    | Proposal: assert the weaker property until 013 lands — the accounts' preferred genres and eras are disjoint, and their positively-rated film sets overlap by at most the FR-016 intersection. Promote to the FR-015 wording once 013 exists |
...
<!--
  ...
  - [x] Open questions are assigned and have a resolution path (1 open, 2 decided)
-->
